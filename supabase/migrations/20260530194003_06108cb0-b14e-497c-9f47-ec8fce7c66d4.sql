-- Recreate event_inquiries view to expose customer_language from v2_events
...
-- Update INSTEAD OF UPDATE trigger to propagate customer_language to v2_events
...
BEGIN
  ...
  UPDATE v2_events SET
    ...
    is_test = NEW.is_test,
    customer_language = COALESCE(NEW.customer_language, 'de'),
    updated_at = now()
  WHERE id = OLD.id;
  ...
  IF NEW.email IS DISTINCT FROM OLD.email
    ...
    UPDATE v2_customers SET
      ...
      updated_at = now()
    WHERE id = (SELECT customer_id FROM v2_events WHERE id = NEW.id);
  END IF;
