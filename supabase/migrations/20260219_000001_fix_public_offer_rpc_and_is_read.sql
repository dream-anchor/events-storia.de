-- Fix 1: get_public_offer WHERE-Klausel erweitern (auch offer_phase berücksichtigen)
...
BEGIN
  ...
  AND (
    ...
    OR ei.offer_phase IN ('proposal_sent', 'customer_responded', 'final_sent', 'confirmed', 'paid')
  );
  RETURN result;
END; $$;
...
-- Fix 2: is_read Spalte auf offer_customer_responses
ALTER TABLE offer_customer_responses ADD COLUMN IF NOT EXISTS is_read boolean DEFAULT false;
