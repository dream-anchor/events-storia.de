-- Migration: get_public_offer liest packageNameOverride aus menu_selection
-- Ermöglicht dem Admin, den Paket-Namen pro Angebot zu überschreiben.
-- Der Override wird in menu_selection.packageNameOverride (JSONB) gespeichert
-- und hat Vorrang über den Original-Paketnamen aus der packages-Tabelle.
  ...
  SELECT json_build_object(
    ...
    'options', COALESCE((
      SELECT json_agg(json_build_object(
        ...
        -- Admin-Override hat Vorrang, dann Paket-Name aus DB, dann Default
        'package_name', CASE
          WHEN ioo.offer_mode = 'menu' THEN 'Individuelles Menü'
          ELSE COALESCE(
            NULLIF(ioo.menu_selection->>'packageNameOverride', ''),
            p.name,
            'Individuelles Paket'
          )
        END,
