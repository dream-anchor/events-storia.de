...
-- ============================================================
-- DEFINITIVE FIX: get_public_offer + get_public_offer_by_slug
-- ============================================================

-- 1. Spalte offer_slug sicherstellen (idempotent)
ALTER TABLE event_inquiries
  ADD COLUMN IF NOT EXISTS offer_slug TEXT UNIQUE;

CREATE INDEX IF NOT EXISTS idx_event_inquiries_offer_slug
  ON event_inquiries(offer_slug) WHERE offer_slug IS NOT NULL;

-- 2. get_public_offer: Robuste Version mit optionalen Feldern
DROP FUNCTION IF EXISTS public.get_public_offer(uuid);
...
-- 3. get_public_offer_by_slug
DROP FUNCTION IF EXISTS public.get_public_offer_by_slug(text);

CREATE OR REPLACE FUNCTION public.get_public_offer_by_slug(slug TEXT)
RETURNS json LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  found_id uuid;
  result json;
BEGIN
  SELECT id INTO found_id FROM event_inquiries WHERE offer_slug = slug;
  IF found_id IS NULL THEN RETURN NULL; END IF;
  SELECT public.get_public_offer(found_id) INTO result;
  RETURN result;
END; $$;

-- 4. GRANT-Berechtigungen
