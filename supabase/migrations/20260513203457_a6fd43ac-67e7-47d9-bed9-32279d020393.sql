...
ALTER TABLE public.v2_events
  ...
  ADD COLUMN IF NOT EXISTS order_confirmed_by_name text,
  ADD COLUMN IF NOT EXISTS order_confirmed_ip text,
  ADD COLUMN IF NOT EXISTS order_confirmed_user_agent text,
  ADD COLUMN IF NOT EXISTS order_confirmed_version integer,
  ADD COLUMN IF NOT EXISTS order_confirmation_terms_version text,
  ADD COLUMN IF NOT EXISTS payment_timing text;

-- 2) Check-Constraint für payment_timing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'v2_events_payment_timing_check'
  ) THEN
    ALTER TABLE public.v2_events
      ADD CONSTRAINT v2_events_payment_timing_check
      CHECK (
        payment_timing IS NULL OR payment_timing IN (
          'online_full','online_deposit','transfer_prepay','on_site','after_event'
        )
      );
  END IF;
END$$;

-- 3) Index für schnelle Abfragen "verbindliche Aufträge"
CREATE INDEX IF NOT EXISTS idx_v2_events_order_confirmed_at
  ON public.v2_events (order_confirmed_at)
  WHERE order_confirmed_at IS NOT NULL;

-- 4) View aktualisieren, damit Admin-UI die neuen Felder sieht.
--    Wir verwenden CREATE OR REPLACE VIEW: alle bestehenden Spalten bleiben in
--    Reihenfolge & Typ erhalten; neue Spalten werden hinten angefügt.
CREATE OR REPLACE VIEW public.event_inquiries AS
 SELECT ev.id,
    ...
    ev.invoice_due_days,
    ev.deposit_amount,
    -- Neue Felder Auftragsbestätigung
