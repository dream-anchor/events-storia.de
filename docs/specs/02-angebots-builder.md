...
+ minimum_spend_cents integer null    -- Mindestumsatz: Anzeige + Warn-Flag bei Endpreis < Minimum; KEINE Auto-Zeile
...
option_id uuid · day_no smallint · label text · date date null · guest_count integer not null
unique (option_id, day_no) · (option_id, tenant_id) → offer_options(id, tenant_id) on delete cascade
```
`offer_items` (NEU):
```
...
course_label text null · name text not null · description text
catalog_item_id uuid null  -- (catalog_item_id, tenant_id) → Katalog, gleiches Muster; Snapshot ohne FK-Zwang
qty_milli integer not null default 1000  -- Tausendstel (330 = 0,33); UI zeigt Dezimal, API Ganzzahl
...
is_optional boolean not null default false  -- Add-on, Public-Seite an-/abwählbar; Persistenz: Modul 03
