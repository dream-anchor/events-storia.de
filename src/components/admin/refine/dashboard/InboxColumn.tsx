  ...
  return (
    <div className="space-y-6">
          ...
          <div className="divide-y divide-destructive/10">
            {overduePayments.slice(0, 5).map(p => (
              <button
                ...
                className="w-full flex items-center justify-between px-2 py-2.5 -mx-2 rounded-lg hover:bg-destructive/10 transition-colors text-sm min-h-[44px]"
              >
                <span className="font-medium text-foreground truncate">{p.customerName}</span>
                <span className="text-xs text-destructive flex-shrink-0 ml-3 tabular-nums">
      ...
      <section>
        ...
        {inbox.length === 0 ? (
          <p className="text-xs text-muted-foreground py-6 text-center">Keine neuen Eingänge</p>
        ) : (
          <div className="divide-y divide-border/40">
            {inbox.slice(0, 8).map(i => (
              <button
                ...
                className="w-full flex items-center gap-3 px-2 py-3 -mx-2 rounded-lg hover:bg-muted/60 transition-colors text-left min-h-[44px]"
              >
          ...
          <div className="divide-y divide-border/40">
            {stale.slice(0, 5).map(s => (
              <button
                ...
                className="w-full flex items-center justify-between px-2 py-3 -mx-2 rounded-lg hover:bg-muted/60 transition-colors min-h-[44px]"
              >
