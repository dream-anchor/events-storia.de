...
export function OfferOptionCard({
  ...
  const calculateTotal = () => {
    ...
    return calculateEventPackagePrice(
      selectedPackage.id,
  ...
  const isLocationPkg = selectedPackage ? isLocationPackage(selectedPackage.id, selectedPackage.price) : false;
  const hasTieredPricing = isLocationPkg && option.guestCount > LOCATION_BASE_GUESTS;
  const pricingBreakdown = isLocationPkg ? getLocationPricingBreakdown(option.guestCount) : null;
        ...
        {selectedPackage && (
          <div className="p-5 rounded-2xl bg-muted/20 border border-border/30 shadow-[var(--shadow-subtle,_0_1px_2px_rgba(0,0,0,0.03))] space-y-3">
            {selectedPackage.price_per_person ? (
              ...
              <>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Basis (bis {LOCATION_BASE_GUESTS} Gäste)</span>
                  <span className="font-medium text-foreground">{pricingBreakdown!.basePrice.toFixed(2)} €</span>
                </div>
                {hasTieredPricing && pricingBreakdown && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">+ {pricingBreakdown.extraGuests} Pers. × {pricingBreakdown.pricePerExtraGuest.toFixed(2)} €</span>
                    <span className="font-medium text-foreground">{pricingBreakdown.extraCost.toFixed(2)} €</span>
                  </div>
                )}
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Gäste</span>
